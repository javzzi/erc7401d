...
contract NestTransferTest is ERC7401DTestBase {
    ...
    function test_ShouldChangeTheDirectOwnerOfTheTokenToTheParentContract() public {
        vm.prank(owner);
        erc7401dChild1.nestTransferFrom(owner, address(erc7401dParent), childTokenId, parentTokenId, "");
        (address directOwner, uint256 tokenId,) = erc7401dChild1.directOwnerOf(childTokenId);
    ...
    function test_ShouldEmitTransferAndNestTransferEventsOnNestTransfer() public {
        vm.prank(owner);
        vm.expectEmit(true, true, true, true);
        emit IERC721.Transfer(owner, address(erc7401dParent), childTokenId);
        vm.expectEmit(true, true, true, true);
        emit IERC7401.NestTransfer(owner, address(erc7401dParent), 0, parentTokenId, childTokenId);
        erc7401dChild1.nestTransferFrom(owner, address(erc7401dParent), childTokenId, parentTokenId, "");
    }
    ...
    function test_RevertWhen_NestTransferringANonExistentToken() public {
        ...
        erc7401dChild1.nestTransferFrom(owner, address(erc7401dParent), 999, parentTokenId, "");
    }

    function test_RevertWhen_NestTransferringATokenFromAnAddressThatIsNotTheOwnerOrApproved() public {
        vm.prank(other);
        vm.expectRevert(ERC7401NotApprovedOrDirectOwner.selector);
        erc7401dChild1.nestTransferFrom(owner, address(erc7401dParent), childTokenId, parentTokenId, "");
    }

    function test_RevertWhen_NestTransferringToANonContractIfTheDestinationIdIsNot0() public {
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(ERC7401DInvalidParentAddress.selector, other));
        erc7401dChild1.nestTransferFrom(owner, other, childTokenId, 1, "");
    }

    function test_RevertWhen_NestTransferringToAContractThatIsNotAParent() public {
        ...
        vm.prank(owner);
        erc7401dChild1.nestTransferFrom(owner, address(newParent), childTokenId, 1, "");
    }

    function test_RevertWhen_NestTransferringToANonExistentParentToken() public {
        vm.prank(owner);
        vm.expectRevert(ERC721InvalidTokenId.selector);
        erc7401dChild1.nestTransferFrom(owner, address(erc7401dParent), childTokenId, 999, "");
    }

    function test_RevertWhen_NestTransferringATokenToItself() public {
        vm.prank(owner);
        vm.expectRevert(ERC7401NestableTransferToSelf.selector);
        erc7401dChild1.nestTransferFrom(owner, address(erc7401dChild1), childTokenId, childTokenId, "");
    }

    function test_RevertWhen_NestTransferringToADestinationThatWouldCreateACircularDependency() public {
        ...
        erc7401dParent.nestTransferFrom(owner, address(erc7401dChild1), parentTokenId, childTokenId, "");
    }
}
...
contract NestTransferFuzzTest is ERC7401DTestBase {
    ...
    function testFuzz_NestTransfer(address from, uint256 tokenIdSeed, uint256 destinationIdSeed) public {
        vm.assume(from != address(0));
        vm.assume(from.code.length == 0);
        uint256 testTokenId = bound(tokenIdSeed, 1000, type(uint128).max);
        uint256 testDestinationId = bound(destinationIdSeed, 2000, type(uint128).max);
        ...
        vm.prank(from);
        erc7401dChild1.nestTransferFrom(from, address(erc7401dParent), testTokenId, testDestinationId, "");

        assertEq(erc7401dChild1.balanceOf(from), fromBalance - 1);
        assertEq(erc7401dChild1.balanceOf(address(erc7401dParent)), toBalance + 1);

        (address directOwner, uint256 parentTokenId,) = erc7401dChild1.directOwnerOf(testTokenId);
        assertEq(directOwner, address(erc7401dParent));
    ...
    function testFuzz_ChildTransfer(uint256 tokenIdSeed, uint256 childIndexSeed, uint256 destinationIdSeed) public {
        uint256 testTokenId = bound(tokenIdSeed, 1000, type(uint128).max);
        uint256 testDestinationId = bound(destinationIdSeed, 2000, type(uint128).max);
        vm.assume(testTokenId != testDestinationId);
        ...
        erc7401dChild1.nestMint(address(erc7401dParent), childTokenId, testTokenId, "");

        uint256[] memory children = erc7401dParent.childrenOf(testTokenId, address(erc7401dChild1));
    ...
    function testFuzz_RevertWhen_NestTransferToSelf(uint256 tokenIdSeed) public {
        uint256 testTokenId = bound(tokenIdSeed, 1000, type(uint128).max);
        vm.assume(!erc7401dParent.exists(testTokenId));
        ...
        vm.prank(owner);
        vm.expectRevert(ERC7401NestableTransferToSelf.selector);
        erc7401dParent.nestTransferFrom(owner, address(erc7401dParent), testTokenId, testTokenId, "");
    }

    function testFuzz_RevertWhen_NestTransferToDescendant(uint256 parentIdSeed, uint256 childIdSeed) public {
        ...
        erc7401dChild1.nestMint(address(erc7401dParent), testChildId, testParentId, "");
        ...
        vm.prank(owner);
        vm.expectRevert(ERC7401NestableTransferToDescendant.selector);
        erc7401dParent.nestTransferFrom(owner, address(erc7401dChild1), testParentId, testChildId, "");
    }

    function testFuzz_RevertWhen_NestTransferNonexistentToken(uint256 tokenIdSeed) public {
        uint256 testTokenId = bound(tokenIdSeed, 1000, type(uint128).max);
        vm.assume(!erc7401dParent.exists(testTokenId));
        ...
        vm.prank(owner);
        vm.expectRevert(ERC721InvalidTokenId.selector);
        erc7401dParent.nestTransferFrom(owner, address(erc7401dChild1), testTokenId, parentTokenId, "");
    }

    function testFuzz_RevertWhen_UnauthorizedNestTransfer(address unauthorized, uint256 tokenIdSeed) public {
    ...
    function invariant_DirectOwnerConsistency() public view {
        ...
        for (uint256 tokenId = 1; tokenId <= 1000; tokenId++) {
            if (erc7401dChild1.exists(tokenId)) {
                (address directOwner, uint256 parentTokenId,) = erc7401dChild1.directOwnerOf(tokenId);
    ...
    function invariant_CircularDependencyPrevention() public view {
        ...
        for (uint256 tokenId = 1; tokenId <= 1000; tokenId++) {
            if (erc7401dChild1.exists(tokenId)) {
                _checkNoCircularDependency(address(erc7401dChild1), tokenId, tokenId, 0);
            }
        }
    }
    ...
    function _checkNoCircularDependency(
        ...
        (address directOwner, uint256 parentTokenId,) = ERC7401DMock(contractAddr).directOwnerOf(currentTokenId);

        if (parentTokenId != 0) {
            assertTrue(parentTokenId != originalTokenId);
            _checkNoCircularDependency(directOwner, originalTokenId, parentTokenId, depth + 1);
        }
    }
}
